...
def decode_bits(bits_buffer: str) -> list[str]:
    ...
    start: int = 0
    bits_high: int = len(bits_buffer) - 1
    for k, v in enumerate(bits_buffer):
        if k == bits_high or v != bits_buffer[k + 1]:
            next_start: int = k + 1
            # Ігноруємо початкові та кінцеві нулі
            if (start > 0 or v == '1') and (k < bits_high or v == '1'):
                size: int = next_start - start
                items.append((v, size))

                sizes: list = signals if v == '1' else pauses
