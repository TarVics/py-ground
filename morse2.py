...
def decode_bits(bits_buffer: str) -> list[str]:
    def bits_enumerator(bits_str: str):
        start: int = 0
        bits_high: int = len(bits_str) - 1
        for k, v in enumerate(bits_str):
            if k == bits_high or v != bits_str[k + 1]:
                next_start: int = k + 1
                # Ігноруємо початкові та кінцеві нулі
                if (start > 0 or v == '1') and (k < bits_high or v == '1'):
                    yield [v, next_start - start]
                start = next_start

    bits_enum = bits_enumerator(bits_buffer)
    ...
    while True:
        data: list | None = next(bits_enum, None)
        ...
        items: list = signals if char == '1' else pauses

        if size not in items:
            items.append(size)

    signals.sort()  # 1*N, 3*N
    pauses.sort()   # 1*N, 3*N, 7*N
